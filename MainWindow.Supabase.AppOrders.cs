...
namespace Fale7_POS
{
    public partial class MainWindow
    {
        ...
        [DataContract]
        private sealed class SupabaseRpcAssignDriverRequest
        {
            [DataMember(Name = "p_order_id")] public string OrderId { get; set; }
            [DataMember(Name = "p_driver_user_id")] public string DriverUserId { get; set; }
            [DataMember(Name = "p_driver_name", EmitDefaultValue = false)] public string DriverName { get; set; }
            [DataMember(Name = "p_driver_phone", EmitDefaultValue = false)] public string DriverPhone { get; set; }
            [DataMember(Name = "p_actor_role", EmitDefaultValue = false)] public string ActorRole { get; set; }
            [DataMember(Name = "p_mark_driver_receipt_printed", EmitDefaultValue = false)] public bool? MarkDriverReceiptPrinted { get; set; }
        }
        ...
        private void QueueSupabaseAppOrdersPullFromBackend()
        {
            Task.Run(async delegate
            {
                ...
                {
                    if (_supabaseAppOrdersPullInFlight)
                    {
                        _supabaseAppOrdersPullPending = true;
                        return;
                    }
                ...
                while (true)
                {
                    ...
                    {
                        Debug.WriteLine("Supabase app_orders pull failed: " + ex.Message);
                    }

                    lock (_supabaseRealtimeSync)
                    {
                        if (_supabaseAppOrdersPullPending)
                        {
                            _supabaseAppOrdersPullPending = false;
                            continue;
                        }

                        _supabaseAppOrdersPullInFlight = false;
                        break;
                    }
                }
            });
        ...
        private bool QueueSupabaseBackendAppDeliveryStatusUpdate(AppDeliveryOrderRecord order, string backendStatus, bool? markKitchenPrinted, bool? markDriverReceiptPrinted)
        {
            ...
            return TryPushAppOrderStatusNowOrQueue(cfg, new AppOrderMutationQueueItem
            {
                ...
                MarkKitchenPrinted = markKitchenPrinted,
                MarkDriverReceiptPrinted = markDriverReceiptPrinted,
        ...
        private bool QueueSupabaseBackendAppPickupStatusUpdate(AppPickupOrderRecord order, string backendStatus, bool? markPartnerReceiptPrinted)
        {
            ...
            return TryPushAppOrderStatusNowOrQueue(cfg, new AppOrderMutationQueueItem
            {
                ...
                MarkPartnerReceiptPrinted = markPartnerReceiptPrinted
            });
        }
        ...
        private bool QueueSupabaseBackendAppDeliveryAddressUpdate(AppDeliveryOrderRecord order, string district, string addressText)
        {
            ...
            EnqueuePendingAppOrderMutation(new AppOrderMutationQueueItem
            {
                OrderId = order.Id,
                OperationKind = "delivery_address",
                CreatedAtMillis = NowUnixMs(),
                District = district,
        ...
        private bool QueueSupabaseBackendAppCustomerDeliveryAddressUpdate(AppDeliveryOrderRecord order, string districtId, DeliveryAddressRecord address, int deliveryFee, string addressId)
        {
            ...
            clonedAddress.District = district;

            EnqueuePendingAppOrderMutation(new AppOrderMutationQueueItem
            {
                OrderId = order.Id,
                OperationKind = "customer_delivery_address",
                CreatedAtMillis = NowUnixMs(),
                CustomerUserId = (order.CustomerUserId ?? string.Empty).Trim(),
                CustomerPhone = (order.Phone ?? string.Empty).Trim(),
                AddressId = (addressId ?? string.Empty).Trim(),
                District = district,
                DistrictId = (districtId ?? string.Empty).Trim(),
                AddressText = BuildAppDeliveryAddressText(clonedAddress),
        ...
        private bool QueueSupabaseBackendAssignDriver(AppDeliveryOrderRecord order, DeliveryDriverRecord driver)
        {
            ...
            return TryPushAppDriverAssignmentNowOrQueue(cfg, new AppOrderMutationQueueItem
            {
                ...
                MarkDriverReceiptPrinted = order.DriverReceiptPrinted
            });
        }

        private void ClearLastAppOrderMutationBackendError()
        {
            lock (_supabaseAppOrderMutationErrorSync)
            {
                _lastAppOrderMutationBackendError = string.Empty;
            }
        }
        ...
        private void RememberAppOrderMutationBackendError(string scope, string details)
        {
            ...
            lock (_supabaseAppOrderMutationErrorSync)
            {
                _lastAppOrderMutationBackendError = message;
            }
        ...
        private string GetAppOrderBackendFailureMessage(string defaultMessage)
        {
            ...
            lock (_supabaseAppOrderMutationErrorSync)
            {
                details = (_lastAppOrderMutationBackendError ?? string.Empty).Trim();
            }
        ...
        private bool TryPushAppOrderStatusNowOrQueue(SupabaseClientConfigRecord cfg, AppOrderMutationQueueItem item)
        {
            ...
            try
            {
                pushedNow = UpdateAppOrderStatusWithFallbackAsync(cfg, new SupabaseRpcUpdateAppOrderStatusRequest
                {
            ...
            catch (Exception ex)
            {
                ...
                RememberAppOrderMutationBackendError(
                    "App order status sync",
                    "order=" + item.OrderId
                    + ", target_status=" + (item.ToStatus ?? string.Empty)
                    + ", error=" + ex.Message);
            ...
            {
                ClearLastAppOrderMutationBackendError();
                // Do NOT pull the snapshot here. The authenticated realtime channel will
                // deliver the UPDATE event and ApplySupabaseRealtimeAppOrderChange will
                // update the card in place. An immediate snapshot pull would trigger the
                // RPC which only returns RECEIVED/PENDING orders and would wipe any order
                // that just advanced past PENDING from the UI.
                EndAppOrderBusyState(item.OrderId);
                return true;
            }
            ...
            if (pushCompletedWithoutTransportError)
            {
                RememberAppOrderMutationBackendError(
                    ...
                    "order=" + item.OrderId
                    + ", target_status=" + (item.ToStatus ?? string.Empty)
                    + ", error=backend_returned_false");
                // Same reasoning: let realtime + periodic pull handle the refresh.
                EndAppOrderBusyState(item.OrderId);
                return false;
            }

            // Mutation queued for offline retry; realtime / periodic pull will reconcile.
            EnqueuePendingAppOrderMutation(item);
            EndAppOrderBusyState(item.OrderId);
            return false;
        }
        ...
        private bool TryPushAppDriverAssignmentNowOrQueue(SupabaseClientConfigRecord cfg, AppOrderMutationQueueItem item)
        {
            ...
            try
            {
                pushedNow = AssignDriverWithFallbackAsync(cfg, new SupabaseRpcAssignDriverRequest
                {
                    OrderId = item.OrderId,
            ...
            catch (Exception ex)
            {
                ...
                RememberAppOrderMutationBackendError(
                    "App order driver sync",
                    "order=" + item.OrderId
                    + ", driver_user_id=" + (item.DriverUserId ?? string.Empty)
                    + ", error=" + ex.Message);
        ...
        private bool WasPendingAppOrderMutationAlreadyApplied(SupabaseClientConfigRecord cfg, AppOrderMutationQueueItem item)
        {
            try
            {
                return WasPendingAppOrderMutationAlreadyAppliedAsync(cfg, item).GetAwaiter().GetResult();
            }
            catch
            {
                return false;
        ...
        private string ResolveBackendDriverUserId(DeliveryDriverRecord driver)
        {
            ...
            // Fallback: map by phone/name to a backend-backed driver that already has AppUserId.
            ...
            for (int i = 0; i < _deliveryDrivers.Count; i++)
            {
                ...
                if (!string.IsNullOrWhiteSpace(phone) && string.Equals(phone, dPhone, StringComparison.OrdinalIgnoreCase))
                {
                    return d.AppUserId.Trim();
                }
        ...
        private async Task<bool> UpdateAppOrderStatusWithFallbackAsync(SupabaseClientConfigRecord cfg, SupabaseRpcUpdateAppOrderStatusRequest payload)
        {
            return await PostSupabaseRpcExpectBoolAsync(cfg, "api_update_app_order_status", payload).ConfigureAwait(false);
        }

        private async Task<bool> AssignDriverWithFallbackAsync(SupabaseClientConfigRecord cfg, SupabaseRpcAssignDriverRequest payload)
        {
            return await PostSupabaseRpcExpectBoolAsync(cfg, "api_assign_app_order_driver", payload).ConfigureAwait(false);
        }
        ...
        private async Task UpdateAppOrderAddressWithFallbackAsync(SupabaseClientConfigRecord cfg, string orderId, string district, string addressText)
        {
            ...
            try
            {
                await PatchAppOrderAddressAsync(cfg, configuredTable, orderId, district, addressText).ConfigureAwait(false);
            }
            catch
            {
                if (string.Equals(configuredTable, "app_orders", StringComparison.OrdinalIgnoreCase))
                {
                    throw;
                }
        ...
        private async Task UpdateCustomerDeliveryAddressAndOrdersWithFallbackAsync(SupabaseClientConfigRecord cfg, AppOrderMutationQueueItem item)
        {
            ...
            try
            {
                await UpdateCustomerDeliveryAddressAndOrdersAsync(cfg, configuredTable, item).ConfigureAwait(false);
            }
            catch
            {
                if (string.Equals(configuredTable, "app_orders", StringComparison.OrdinalIgnoreCase))
        ...
        private async Task UpdateCustomerDeliveryAddressAndOrdersAsync(SupabaseClientConfigRecord cfg, string ordersTable, AppOrderMutationQueueItem item)
        {
            ...
            if ((rows == null || rows.Count == 0)
                && !string.IsNullOrWhiteSpace((item.CustomerUserId ?? string.Empty).Trim())
            ...
            for (int i = 0; i < rows.Count; i++)
            {
                ...
                await PatchAppOrderAddressAsync(
                    cfg,
                    ordersTable,
                    row.Id,
                    item.District,
                    ...
                    desiredFee,
                    nextTotal,
                    BuildUpdatedAppOrderAddressMetadata(row, item)).ConfigureAwait(false);
            }
        ...
        private async Task<List<SupabaseAppOrderRow>> FetchCustomerDeliveryOrdersForAddressUpdateAsync(SupabaseClientConfigRecord cfg, string tableName, AppOrderMutationQueueItem item)
        {
            ...
            filters.Add("order_type=eq.DELIVERY");
            ...
            {
                filters.Add("customer_user_id=eq." + Uri.EscapeDataString(customerUserId));
            }
            else if (!string.IsNullOrWhiteSpace(customerPhone))
            {
                filters.Add("customer_phone=eq." + Uri.EscapeDataString(customerPhone));
            }
            else if (!string.IsNullOrWhiteSpace(orderId))
            {
                filters.Add("id=eq." + Uri.EscapeDataString(orderId));
            }
            else
            {
                return new List<SupabaseAppOrderRow>();
            }
        ...
        private async Task SyncCustomerAddressAfterCashierEditAsync(SupabaseClientConfigRecord cfg, AppOrderMutationQueueItem item)
        {
            ...
            if (string.IsNullOrWhiteSpace(customerPhone) || string.IsNullOrWhiteSpace((item.DistrictId ?? string.Empty).Trim()))
            {
                return;
            }
            ...
            if (IsSupabaseAddressId(queuedAddressId))
            {
                ...
                {
                    var row = existing[i];
                    if (row != null && string.Equals(row.Id.ToString(), queuedAddressId, StringComparison.Ordinal))
                    {
                        target = row;
                        break;
                    }
                ...
                if (target == null)
                {
                    target = await FetchCustomerAddressByIdAsync(cfg, queuedAddressId).ConfigureAwait(false);
                }
            }
            if (target == null)
            {
                for (int i = 0; i < existing.Count; i++)
                {
                    ...
                    if (!string.Equals((row.DistrictId ?? string.Empty).Trim(), (item.DistrictId ?? string.Empty).Trim(), StringComparison.Ordinal)) continue;
                    if (!string.Equals((row.Block ?? string.Empty).Trim(), (item.AddressBlock ?? string.Empty).Trim(), StringComparison.Ordinal)) continue;
                    if (!string.Equals((row.Street ?? string.Empty).Trim(), (item.AddressStreet ?? string.Empty).Trim(), StringComparison.Ordinal)) continue;
                    if (!string.Equals((row.Building ?? string.Empty).Trim(), (item.AddressBuilding ?? string.Empty).Trim(), StringComparison.Ordinal)) continue;
                    if (!string.Equals((row.Apartment ?? string.Empty).Trim(), (item.AddressApartment ?? string.Empty).Trim(), StringComparison.Ordinal)) continue;
                    target = row;
                    break;
                }
            }
            ...
            if (target != null && shouldBeDefault)
            {
                await ClearDefaultCustomerAddressesAsync(cfg, effectiveCustomerUserId, effectiveCustomerPhone, target.Id).ConfigureAwait(false);
            }
            ...
            var payload = new SupabaseCustomerAddressPatchRow
            {
                ...
                Label = target != null && !string.IsNullOrWhiteSpace(target.Label)
                    ? target.Label.Trim()
                    : (string.IsNullOrWhiteSpace(item.District) ? "عنوان" : item.District.Trim()),
                DistrictId = (item.DistrictId ?? string.Empty).Trim(),
                Block = item.AddressBlock ?? string.Empty,
                Street = item.AddressStreet ?? string.Empty,
                Building = item.AddressBuilding ?? string.Empty,
                Apartment = item.AddressApartment ?? string.Empty,
                Note = item.AddressNote ?? string.Empty,
                IsDefault = shouldBeDefault
            };

            if (target == null)
            {
                await InsertCustomerAddressAsync(cfg, payload).ConfigureAwait(false);
                return;
            }
        ...
        private async Task<List<SupabaseCustomerAddressRow>> FetchCustomerAddressesForOwnerAsync(SupabaseClientConfigRecord cfg, string customerUserId, string customerPhone)
        {
            ...
            customerUserId = (customerUserId ?? string.Empty).Trim();
            ...
            {
                return new List<SupabaseCustomerAddressRow>();
            }
        ...
        private async Task<List<SupabaseAppOrderRow>> FetchSupabaseAppOrdersAsync(SupabaseClientConfigRecord cfg)
        {
            ...
            catch (Exception ex)
            {
                    ...
                    || message.IndexOf("permission denied", StringComparison.OrdinalIgnoreCase) >= 0)
                {
                    ...
                    try
                    {
                        var retriedRpcRows = await fetchFromRpc().ConfigureAwait(false);
                        if (retriedRpcRows != null) return retriedRpcRows;
                        return await fetchFromTable(configuredTable).ConfigureAwait(false);
                    }
                    catch
                    {
        ...
        private SupabaseAppOrderMetadataRow CloneSupabaseAppOrderMetadata(SupabaseAppOrderMetadataRow source)
        {
            ...
            {
                LocalOrderId = source.LocalOrderId,
                LocalOrderNo = source.LocalOrderNo,
                FinalizedAtMillis = source.FinalizedAtMillis,
                Source = source.Source,
                PosOrderType = source.PosOrderType,
                ...
                DriverNameCamel = source.DriverNameCamel,
                DriverPhoneCamel = source.DriverPhoneCamel,
                AddressId = source.AddressId,
                DistrictId = source.DistrictId,
                DistrictName = source.DistrictName,
                AddressLabel = source.AddressLabel,
                AddressBlock = source.AddressBlock,
                AddressStreet = source.AddressStreet,
                AddressBuilding = source.AddressBuilding,
                AddressApartment = source.AddressApartment,
                AddressNote = source.AddressNote,
                AddressTextFull = source.AddressTextFull
            };
            ...
            if (source.Items != null)
            {
                ...
                for (int i = 0; i < source.Items.Count; i++)
                {
                    ...
                    clone.Items.Add(new SupabaseAppOrderItemRow
                    {
                        Name = item.Name,
                        Qty = item.Qty,
                        Price = item.Price
        ...
        private SupabaseAppOrderMetadataRow BuildUpdatedAppOrderAddressMetadata(SupabaseAppOrderRow row, AppOrderMutationQueueItem item)
        {
            ...
            metadata.AddressLabel = string.IsNullOrWhiteSpace(metadata.AddressLabel)
                ? (string.IsNullOrWhiteSpace(item.District) ? "عنوان العميل" : item.District.Trim())
                : metadata.AddressLabel.Trim();
            metadata.AddressBlock = item.AddressBlock ?? string.Empty;
            metadata.AddressStreet = item.AddressStreet ?? string.Empty;
            metadata.AddressBuilding = item.AddressBuilding ?? string.Empty;
            metadata.AddressApartment = item.AddressApartment ?? string.Empty;
        ...
        private async Task<bool> PostSupabaseRpcExpectBoolAsync<TReq>(SupabaseClientConfigRecord cfg, string rpcName, TReq payload)
        {
            ...
            if (string.IsNullOrWhiteSpace(GetCurrentSessionToken()))
            {
                throw new InvalidOperationException("RPC " + rpcName + " requires an active backend session.");
            }
            ...
            Func<Task<string>> post = async delegate
            {
                using (var req = new HttpRequestMessage(HttpMethod.Post, url))
                {
                    ...
                    using (var resp = await SupabaseHttpClient.SendAsync(req, cts.Token).ConfigureAwait(false))
                    {
                        ...
                        if (!resp.IsSuccessStatusCode)
                        {
                            throw new InvalidOperationException("RPC " + rpcName + " failed: HTTP " + (int)resp.StatusCode + " " + responseRaw);
                        }
                        return responseRaw;
                    }
                }
            ...
            {
                if (!IsSupabaseSessionRetryableError(ex.Message))
                {
                    throw;
                }
            ...
            bool parsedValue;
            if (!TryParseSupabaseRpcBool(raw, out parsedValue))
            {
                throw new InvalidOperationException("RPC " + rpcName + " returned unexpected payload: " + raw.Trim());
            }
            ...
            if (string.IsNullOrWhiteSpace(GetCurrentSessionToken()))
            {
                throw new InvalidOperationException("RPC " + rpcName + " backend session refresh failed.");
            }

            raw = await post().ConfigureAwait(false);
            if (!TryParseSupabaseRpcBool(raw, out parsedValue))
            {
                throw new InvalidOperationException("RPC " + rpcName + " returned unexpected payload after refresh: " + raw.Trim());
            }
        ...
        /// <summary>استدعاء RPC من Supabase وإرجاع نص الاستجابة.</summary>
        private async Task<string> PostSupabaseRpcGetStringAsync(
            SupabaseClientConfigRecord cfg,
            string rpcName,
            string requestBody,
            string actorRoleOverride = null,
            string actorUserIdOverride = null,
            bool throwOnHttpError = false)
        {
            ...
            using (var req = new HttpRequestMessage(HttpMethod.Post, url))
            {
                ...
                {
                    req.Headers.Remove("x-fale7-role");
                    req.Headers.Remove("x_fale7_role");
                    req.Headers.TryAddWithoutValidation("x-fale7-role", overrideRole);
                    req.Headers.TryAddWithoutValidation("x_fale7_role", overrideRole);
                }

                var overrideUserId = (actorUserIdOverride ?? string.Empty).Trim();
                if (!string.IsNullOrWhiteSpace(overrideUserId))
                {
                    req.Headers.Remove("x-fale7-user-id");
                    req.Headers.Remove("x_fale7_user_id");
                    req.Headers.TryAddWithoutValidation("x-fale7-user-id", overrideUserId);
                    req.Headers.TryAddWithoutValidation("x_fale7_user_id", overrideUserId);
                }
                ...
                using (var resp = await SupabaseHttpClient.SendAsync(req, cts.Token).ConfigureAwait(false))
                {
                    ...
                    if (!resp.IsSuccessStatusCode)
                    {
                        if (throwOnHttpError)
                        {
                            throw new InvalidOperationException("RPC " + rpcName + " failed: HTTP " + (int)resp.StatusCode + " " + (raw ?? string.Empty).Trim());
                        }
                        return null;
                    }
                    return raw;
        ...
        private void ApplySupabaseShiftArchiveRows(List<SupabaseAppOrderRow> rows)
        {
            ...
            for (int i = 0; i < rows.Count; i++)
            {
                ApplySupabaseShiftArchiveRow(rows[i]);
            }
        }
        ...
        private void ApplySupabaseShiftArchiveRow(SupabaseAppOrderRow row)
        {
            ...
            if (status == "MIGRATED")
            {
                RemoveTakeawayShiftRecordsByBackendIds(new List<string> { backendOrderId });
                return;
            }
            ...
            if (effectiveType == "PICKUP" || effectiveType == "POS_PICKUP")
            {
                channelKind = "PICKUP";
            }
            else if (effectiveType == "DELIVERY" || effectiveType == "POS_DELIVERY")
            {
                channelKind = "DELIVERY";
            }
            else if (effectiveType == "TAKEAWAY" || effectiveType == "POS_TAKEAWAY")
            {
                channelKind = "TAKEAWAY";
            }
            ...
            if (effectiveType.StartsWith("POS_", StringComparison.OrdinalIgnoreCase)
                || metadataSource == "POS"
            ...
            if (orderNo <= 0 && row.Metadata != null && row.Metadata.LocalOrderNo > 0)
            {
                orderNo = row.Metadata.LocalOrderNo;
            }
            var createdAtMillis = ParseIsoDateToUnixMs(row.CreatedAt);
            var finalizedAtMillis = row.Metadata != null && row.Metadata.FinalizedAtMillis > 0
                ? row.Metadata.FinalizedAtMillis
                : ParseIsoDateToUnixMs(row.UpdatedAt);
            ...
            var record = new PosShiftOrderRecord
            {
                ...
                OrderNo = orderNo,
                DisplayRef = orderNo > 0
                    ? orderNo.ToString()
                    : (!string.IsNullOrWhiteSpace(row.Metadata == null ? null : row.Metadata.LocalOrderId)
                        ? row.Metadata.LocalOrderId
                        : backendOrderId),
        ...
        private HashSet<string> BuildPendingPosShiftMigrationOrderIdsSet()
        {
            ...
            if (pending == null || pending.Count == 0) return set;

            for (int i = 0; i < pending.Count; i++)
            {
                var raw = (pending[i] ?? string.Empty).Trim();
                if (string.IsNullOrWhiteSpace(raw)) continue;
                set.Add(raw);

                var canonical = raw.StartsWith("pos_", StringComparison.OrdinalIgnoreCase)
                    ? raw
                    : BuildPosOfflineBackendOrderId(raw);
                if (!string.IsNullOrWhiteSpace(canonical)) set.Add(canonical.Trim());
            }
        ...
        private bool IsHiddenByPendingMigration(HashSet<string> pendingIds, string orderId)
        {
            ...
            var canonical = id.StartsWith("pos_", StringComparison.OrdinalIgnoreCase)
                ? id
        ...
        private bool RemoveLocalAppOrderById(string orderId)
        {
            ...
            {
                var existing = _deliveryAppDeliveryOrders[i];
                if (existing == null || !string.Equals(existing.Id, id, StringComparison.OrdinalIgnoreCase)) continue;
            ...
            {
                var existing = _deliveryAppPickupOrders[i];
                if (existing == null || !string.Equals(existing.Id, id, StringComparison.OrdinalIgnoreCase)) continue;
                _deliveryAppPickupOrders.RemoveAt(i);
                removed = true;
            }

            if (_deliveryAppDeliverySelectedIds != null)
            {
                var removedSelection = _deliveryAppDeliverySelectedIds.RemoveWhere(delegate (string selectedId)
                {
                    return string.Equals(selectedId, id, StringComparison.OrdinalIgnoreCase);
                });
                if (removedSelection > 0) removed = true;
            }

            if (_deliveryAppPickupSelectedIds != null)
            {
                var removedSelection = _deliveryAppPickupSelectedIds.RemoveWhere(delegate (string selectedId)
                {
                    return string.Equals(selectedId, id, StringComparison.OrdinalIgnoreCase);
                });
                if (removedSelection > 0) removed = true;
        ...
        private void ApplySupabaseAppOrdersToLocalUi(List<SupabaseAppOrderRow> rows)
        {
            if (!Dispatcher.CheckAccess())
            {
                ...
                {
                    try { ApplySupabaseAppOrdersToLocalUi(rows); } catch (Exception ex) { Debug.WriteLine("Supabase app_orders UI apply failed: " + ex.Message); }
            ...
            for (int i = 0; i < rows.Count; i++)
            {
                var row = rows[i];
                if (row == null) continue;
                if (IsHiddenByPendingMigration(pendingMigrationIds, row.Id)) continue;
                visibleRows.Add(row);
            }
            ...
            for (int i = 0; i < visibleRows.Count; i++)
            {
                ...
                if (!string.IsNullOrWhiteSpace(row.Id))
                {
                    ...
                    incomingOrderIds.Add(rowId);
                    if (_supabaseAppOrdersSeenPrimed && !_supabaseSeenAppOrderIds.Contains(rowId))
                    {
                        var normalizedType = (row.OrderType ?? string.Empty).Trim().ToUpperInvariant();
                        var normalizedStatus = (row.Status ?? string.Empty).Trim().ToUpperInvariant();
                        if (normalizedStatus == "RECEIVED" || normalizedStatus == "PENDING")
                        {
                            var orderRef = DeriveSupabaseOrderNo(row).ToString();
                            if (normalizedType == "DELIVERY")
                            {
                                newDeliveryOrderRefs.Add(orderRef);
                            }
                            else if (normalizedType == "PICKUP")
                            {
                                newPickupOrderRefs.Add(orderRef);
                            }
                        }
                    }
                }
                ...
                {
                    var mapped = MapSupabaseDeliveryOrderRow(row);
                    if (mapped == null) continue;
                    delivery.Add(mapped);
                    if (mapped.No > maxDeliveryNo) maxDeliveryNo = mapped.No;
                    continue;
                }
                if (orderType == "PICKUP")
                {
                    var mapped = MapSupabasePickupOrderRow(row);
                    if (mapped == null) continue;
                    pickup.Add(mapped);
                    if (mapped.No > maxPickupNo) maxPickupNo = mapped.No;
                    continue;
                }
            }

            // ── Smart Merge ────────────────────────────────────────────────────────────
            // The RPC snapshot (api_pos_app_orders_snapshot) only returns orders in
            // RECEIVED / PENDING status. Once a cashier advances an order to PREPARING
            // or beyond, the RPC stops including it. A raw Clear() + full-replace would
            // wipe those active orders from the UI even though they still exist in the DB.
            //
            // Strategy:
            //   1. Scan the current in-memory collections for orders whose ID is NOT
            //      present in the incoming snapshot.
            //   2. If such an order has a non-terminal status, RETAIN it — it was
            //      filtered by the RPC, not deleted from the database.
            //   3. Terminal orders (Done / Delivered) that are absent from the snapshot
            //      have genuinely finished; discard them.
            //   4. Merge: effective list = fresh incoming rows + retained active rows.
            //   5. Compute the snapshot key from the effective list so the change
            //      detection logic accounts for retained orders.
            // ───────────────────────────────────────────────────────────────────────────

            // Retain active delivery orders not covered by this snapshot.
            var retainedDelivery = new List<AppDeliveryOrderRecord>();
            for (int i = 0; i < _deliveryAppDeliveryOrders.Count; i++)
            {
                var existing = _deliveryAppDeliveryOrders[i];
                if (existing == null || string.IsNullOrWhiteSpace(existing.Id)) continue;
                if (incomingOrderIds.Contains(existing.Id.Trim())) continue; // superseded by fresh data
                if (existing.Status == AppDeliveryOrderStatus.Done) continue; // terminal → discard
                retainedDelivery.Add(existing);
            }

            // Retain active pickup orders not covered by this snapshot.
            var retainedPickup = new List<AppPickupOrderRecord>();
            for (int i = 0; i < _deliveryAppPickupOrders.Count; i++)
            {
                var existing = _deliveryAppPickupOrders[i];
                if (existing == null || string.IsNullOrWhiteSpace(existing.Id)) continue;
                if (incomingOrderIds.Contains(existing.Id.Trim())) continue;
                if (existing.Status == AppPickupOrderStatus.Delivered) continue; // terminal → discard
                retainedPickup.Add(existing);
            }

            // Build effective lists: incoming fresh data first, then retained actives.
            var effectiveDelivery = new List<AppDeliveryOrderRecord>(delivery.Count + retainedDelivery.Count);
            effectiveDelivery.AddRange(delivery);
            effectiveDelivery.AddRange(retainedDelivery);

            var effectivePickup = new List<AppPickupOrderRecord>(pickup.Count + retainedPickup.Count);
            effectivePickup.AddRange(pickup);
            effectivePickup.AddRange(retainedPickup);

            // Extend sequence numbers to cover any retained orders.
            for (int i = 0; i < retainedDelivery.Count; i++)
                if (retainedDelivery[i].No > maxDeliveryNo) maxDeliveryNo = retainedDelivery[i].No;
            for (int i = 0; i < retainedPickup.Count; i++)
                if (retainedPickup[i].No > maxPickupNo) maxPickupNo = retainedPickup[i].No;

            // Change detection is now based on the effective (merged) list.
            var nextSnapshotKey = BuildAppOrdersUiSnapshotKey(effectiveDelivery, effectivePickup);
            var uiChanged = !string.Equals(_supabaseAppOrdersUiSnapshotKey, nextSnapshotKey, StringComparison.Ordinal);

            if (uiChanged)
            {
                // Apply the effective merged list — never blindly wipe active orders.
                _deliveryAppDeliveryOrders.Clear();
                for (int i = 0; i < effectiveDelivery.Count; i++) _deliveryAppDeliveryOrders.Add(effectiveDelivery[i]);

                _deliveryAppPickupOrders.Clear();
                for (int i = 0; i < effectivePickup.Count; i++) _deliveryAppPickupOrders.Add(effectivePickup[i]);

                _deliveryAppDeliveryOrderSequence = maxDeliveryNo;
                _deliveryAppPickupOrderSequence = maxPickupNo;
                _deliveryAppOrdersInitialized = true;
                _supabaseAppOrdersUiSnapshotKey = nextSnapshotKey;
                PersistDeliveryState();

                if (_deliveryAppDeliverySelectedIds != null)
                {
                    _deliveryAppDeliverySelectedIds.RemoveWhere(delegate(string id) { return !ContainsAppDeliveryOrderId(id); });
                }
                if (_deliveryAppPickupSelectedIds != null)
                {
                    _deliveryAppPickupSelectedIds.RemoveWhere(delegate(string id) { return !ContainsAppPickupOrderId(id); });
                }
            ...
            if (!_supabaseAppOrdersSeenPrimed)
            {
                _supabaseAppOrdersSeenPrimed = true;
                return;
            }
            ...
            if (newDeliveryOrderRefs.Count > 0 || newPickupOrderRefs.Count > 0)
            {
                ...
                if (newDeliveryOrderRefs.Count > 0)
                {
                    parts.Add("دليفري: " + string.Join(", ", newDeliveryOrderRefs));
                }
                if (newPickupOrderRefs.Count > 0)
                {
                    parts.Add("استلام: " + string.Join(", ", newPickupOrderRefs));
                }
        ...
        private string BuildAppOrdersUiSnapshotKey(List<AppDeliveryOrderRecord> delivery, List<AppPickupOrderRecord> pickup)
        {
            ...
            delivery = delivery ?? new List<AppDeliveryOrderRecord>();
            pickup = pickup ?? new List<AppPickupOrderRecord>();
            ...
            for (int i = 0; i < delivery.Count; i++)
            {
                ...
                sb.Append('|')
                  .Append(order.Id ?? string.Empty)
                  .Append('|').Append(order.No)
                  .Append('|').Append(order.AddressId ?? string.Empty)
                  .Append('|').Append(order.Status)
                  .Append('|').Append(order.District ?? string.Empty)
                  .Append('|').Append(order.AddressText ?? string.Empty)
                  .Append('|').Append(order.DeliveryFee)
                  .Append('|').Append(order.Total)
                  .Append('|').Append(order.KitchenPrinted ? "1" : "0")
                  .Append('|').Append(order.DriverReceiptPrinted ? "1" : "0")
                  .Append('|').Append(order.Driver == null ? string.Empty : (order.Driver.Id ?? string.Empty));
            }
            ...
            for (int i = 0; i < pickup.Count; i++)
            {
                ...
                sb.Append('|')
                  ...
                  .Append('|').Append(order.District ?? string.Empty)
                  .Append('|').Append(order.AddressText ?? string.Empty)
                  .Append('|').Append(order.PartnerReceiptPrinted ? "1" : "0");
            }
        ...
        private bool AreAppOrderItemsEquivalent(List<AppOrderItemRecord> left, List<AppOrderItemRecord> right)
        {
            ...
            left = left ?? new List<AppOrderItemRecord>();
            right = right ?? new List<AppOrderItemRecord>();
            ...
            for (int i = 0; i < left.Count; i++)
            {
                ...
                if (!string.Equals(a.Name ?? string.Empty, b.Name ?? string.Empty, StringComparison.Ordinal)) return false;
                if (a.Qty != b.Qty || a.Price != b.Price) return false;
            }
        ...
        private bool AreAppDeliveryOrdersEquivalentForRealtime(AppDeliveryOrderRecord left, AppDeliveryOrderRecord right)
        {
            ...
            var leftDriver = left.Driver;
            var rightDriver = right.Driver;
            if ((leftDriver == null) != (rightDriver == null)) return false;
            if (leftDriver != null)
            {
                if (!string.Equals(leftDriver.Id ?? string.Empty, rightDriver.Id ?? string.Empty, StringComparison.Ordinal)) return false;
                if (!string.Equals(leftDriver.Name ?? string.Empty, rightDriver.Name ?? string.Empty, StringComparison.Ordinal)) return false;
                if (!string.Equals(leftDriver.Phone ?? string.Empty, rightDriver.Phone ?? string.Empty, StringComparison.Ordinal)) return false;
                if (!string.Equals(leftDriver.AppUserId ?? string.Empty, rightDriver.AppUserId ?? string.Empty, StringComparison.Ordinal)) return false;
            }
        ...
        private void ApplySupabaseRealtimeAppOrderChange(string eventType, SupabaseAppOrderRow record, SupabaseAppOrderRow oldRecord)
        {
            if (!Dispatcher.CheckAccess())
            {
                Dispatcher.BeginInvoke(new Action(delegate
                {
                    try { ApplySupabaseRealtimeAppOrderChange(eventType, record, oldRecord); } catch (Exception ex) { Debug.WriteLine("Supabase realtime app_order apply failed: " + ex.Message); }
                }), DispatcherPriority.Background);
                return;
            }
            ...
            {
                if (ContainsAppDeliveryOrderId(rowId)) orderType = "DELIVERY";
                else if (ContainsAppPickupOrderId(rowId)) orderType = "PICKUP";
            }
            if (string.IsNullOrWhiteSpace(orderType)) return;

            var pendingMigrationIds = BuildPendingPosShiftMigrationOrderIdsSet();
            if (IsHiddenByPendingMigration(pendingMigrationIds, rowId))
            {
                var removed = RemoveLocalAppOrderById(rowId);
                _supabaseAppOrdersSeenPrimed = true;
                if (removed)
                {
                    PersistDeliveryState();
            ...
            var isNewPending = false;
            var newOrderRef = string.Empty;
            var changed = false;

            if (string.Equals(orderType, "DELIVERY", StringComparison.Ordinal))
            {
                if (isDelete)
                {
                    for (int i = _deliveryAppDeliveryOrders.Count - 1; i >= 0; i--)
                    {
                        var existing = _deliveryAppDeliveryOrders[i];
                        if (existing != null && string.Equals(existing.Id, rowId, StringComparison.OrdinalIgnoreCase))
                        {
                            _deliveryAppDeliveryOrders.RemoveAt(i);
                            changed = true;
                        }
                ...
                {
                    var mapped = MapSupabaseDeliveryOrderRow(source);
                    if (mapped != null)
                    {
                        var replaced = false;
                        for (int i = 0; i < _deliveryAppDeliveryOrders.Count; i++)
                        {
                            var existing = _deliveryAppDeliveryOrders[i];
                            if (existing == null || !string.Equals(existing.Id, mapped.Id, StringComparison.OrdinalIgnoreCase)) continue;
                            if (!AreAppDeliveryOrdersEquivalentForRealtime(existing, mapped))
                            {
                                _deliveryAppDeliveryOrders[i] = mapped;
                                changed = true;
                            }
                            replaced = true;
                            break;
                        ...
                        _deliveryAppDeliveryOrderSequence = Math.Max(_deliveryAppDeliveryOrderSequence, mapped.No);

                        var status = (source.Status ?? string.Empty).Trim().ToUpperInvariant();
                        if (seenPrimedAtStart &&
                            !_supabaseSeenAppOrderIds.Contains(rowId) &&
                            (string.Equals(status, "RECEIVED", StringComparison.Ordinal) || string.Equals(status, "PENDING", StringComparison.Ordinal)))
                        {
                            isNewPending = true;
                            newOrderRef = mapped.No.ToString();
                        }
                    }
                }
            }
            else if (string.Equals(orderType, "PICKUP", StringComparison.Ordinal))
            {
                if (isDelete)
                {
                    for (int i = _deliveryAppPickupOrders.Count - 1; i >= 0; i--)
                ...
                {
                    var mapped = MapSupabasePickupOrderRow(source);
                    if (mapped != null)
                    {
                        var replaced = false;
                        for (int i = 0; i < _deliveryAppPickupOrders.Count; i++)
                        {
                            var existing = _deliveryAppPickupOrders[i];
                            if (existing == null || !string.Equals(existing.Id, mapped.Id, StringComparison.OrdinalIgnoreCase)) continue;
                            if (!AreAppPickupOrdersEquivalentForRealtime(existing, mapped))
                            {
                                _deliveryAppPickupOrders[i] = mapped;
                                changed = true;
                            }
                            replaced = true;
                            break;
                        ...
                        _deliveryAppPickupOrderSequence = Math.Max(_deliveryAppPickupOrderSequence, mapped.No);
            ...
            if (isDelete) _supabaseSeenAppOrderIds.Remove(rowId);
            else _supabaseSeenAppOrderIds.Add(rowId);
            ...
            _deliveryAppDeliveryOrders.Sort(delegate (AppDeliveryOrderRecord a, AppDeliveryOrderRecord b)
            {
                var aCreated = a == null ? 0L : a.CreatedAt;
                var bCreated = b == null ? 0L : b.CreatedAt;
                return aCreated.CompareTo(bCreated);
            });
            _deliveryAppPickupOrders.Sort(delegate (AppPickupOrderRecord a, AppPickupOrderRecord b)
            {
                var aCreated = a == null ? 0L : a.CreatedAt;
                var bCreated = b == null ? 0L : b.CreatedAt;
                return aCreated.CompareTo(bCreated);
            ...
            if (isNewPending && !string.IsNullOrWhiteSpace(newOrderRef))
            {
                if (string.Equals(orderType, "DELIVERY", StringComparison.Ordinal))
                {
                    ShowSupabaseToast("طلب تطبيق جديد", "دليفري: " + newOrderRef, true);
                }
                else if (string.Equals(orderType, "PICKUP", StringComparison.Ordinal))
                {
                    ShowSupabaseToast("طلب تطبيق جديد", "استلام: " + newOrderRef, true);
                }
            }
        }

        private bool ContainsAppDeliveryOrderId(string id)
        {
            if (string.IsNullOrWhiteSpace(id)) return false;
            for (int i = 0; i < _deliveryAppDeliveryOrders.Count; i++)
            {
                var o = _deliveryAppDeliveryOrders[i];
                if (o != null && string.Equals(o.Id, id, StringComparison.OrdinalIgnoreCase)) return true;
            }
            return false;
        }

        private bool ContainsAppPickupOrderId(string id)
        {
            if (string.IsNullOrWhiteSpace(id)) return false;
            for (int i = 0; i < _deliveryAppPickupOrders.Count; i++)
            {
        ...
        private DeliveryAddressRecord BuildAppOrderAddressFromMetadata(string district, string addressText, SupabaseAppOrderMetadataRow metadata)
        {
            ...
            if (!string.IsNullOrWhiteSpace(metadata.AddressTextFull))
            {
                var parsedFromFullText = ParseAppDeliveryAddressText(district, metadata.AddressTextFull);
                if (parsedFromFullText != null)
                {
                    if (string.IsNullOrWhiteSpace(merged.Block) && !string.IsNullOrWhiteSpace(parsedFromFullText.Block)) merged.Block = parsedFromFullText.Block;
                    if (string.IsNullOrWhiteSpace(merged.Street) && !string.IsNullOrWhiteSpace(parsedFromFullText.Street)) merged.Street = parsedFromFullText.Street;
                    if (string.IsNullOrWhiteSpace(merged.Building) && !string.IsNullOrWhiteSpace(parsedFromFullText.Building)) merged.Building = parsedFromFullText.Building;
                    if (string.IsNullOrWhiteSpace(merged.Apartment) && !string.IsNullOrWhiteSpace(parsedFromFullText.Apartment)) merged.Apartment = parsedFromFullText.Apartment;
                    if (string.IsNullOrWhiteSpace(merged.Floor) && !string.IsNullOrWhiteSpace(parsedFromFullText.Floor)) merged.Floor = parsedFromFullText.Floor;
        ...
        {
            if (metadata != null && !string.IsNullOrWhiteSpace(metadata.AddressTextFull))
            {
                return metadata.AddressTextFull.Trim();
            }
        ...
        private AppDeliveryOrderRecord MapSupabaseDeliveryOrderRow(SupabaseAppOrderRow row)
        {
            ...
            var metaDriverName = row.Metadata == null
                ? string.Empty
                : (!string.IsNullOrWhiteSpace(row.Metadata.DriverName) ? row.Metadata.DriverName : row.Metadata.DriverNameCamel);
            var metaDriverPhone = row.Metadata == null
                ? string.Empty
                : (!string.IsNullOrWhiteSpace(row.Metadata.DriverPhone) ? row.Metadata.DriverPhone : row.Metadata.DriverPhoneCamel);
            ...
            var driverId = !string.IsNullOrWhiteSpace(row.DriverUserId) ? row.DriverUserId
                : (!string.IsNullOrWhiteSpace(driverPhone) ? ("phone:" + driverPhone.Trim()) : "APP_DRIVER");
            var subtotal = (int)Math.Round(row.Subtotal < 0 ? 0 : row.Subtotal, MidpointRounding.AwayFromZero);
            var deliveryFee = (int)Math.Round(row.DeliveryFee < 0 ? 0 : row.DeliveryFee, MidpointRounding.AwayFromZero);
            var totalRaw = row.Total > 0 ? row.Total : (row.Subtotal + row.DeliveryFee);
            var total = (int)Math.Round(totalRaw < 0 ? 0 : totalRaw, MidpointRounding.AwayFromZero);
            var driver = string.IsNullOrWhiteSpace(row.DriverUserId) && string.IsNullOrWhiteSpace(driverName) && string.IsNullOrWhiteSpace(driverPhone)
                ? null
                : new DeliveryDriverRecord
                {
                    Id = driverId,
                    Name = ResolveFriendlyDriverName(driverId, driverName, driverPhone),
                    Phone = driverPhone ?? string.Empty,
                    AppUserId = row.DriverUserId
                };
            ...
            return new AppDeliveryOrderRecord
            {
                Id = row.Id,
                CustomerUserId = row.CustomerUserId,
                AddressId = row.Metadata == null ? string.Empty : (row.Metadata.AddressId ?? string.Empty),
                No = DeriveSupabaseOrderNo(row),
                Name = row.CustomerName ?? string.Empty,
                Phone = row.CustomerPhone ?? string.Empty,
                District = string.IsNullOrWhiteSpace(address.District) ? (row.District ?? string.Empty) : address.District,
                AddressText = preferredAddressText,
                Address = address,
                CreatedAt = ParseIsoDateToUnixMs(row.CreatedAt),
                Subtotal = subtotal,
                DeliveryFee = deliveryFee,
                Total = total,
                Status = status,
                ...
                Driver = driver,
                Items = MapSupabaseItems(row.Items, row.Metadata == null ? null : row.Metadata.Items)
            };
        }

        private AppPickupOrderRecord MapSupabasePickupOrderRow(SupabaseAppOrderRow row)
        {
            if (row == null || string.IsNullOrWhiteSpace(row.Id)) return null;
            var statusRaw = (row.Status ?? string.Empty).Trim().ToUpperInvariant();
            if (statusRaw == "MIGRATED") return null;
            AppPickupOrderStatus status;
            if (statusRaw == "RECEIVED" || statusRaw == "PENDING") status = AppPickupOrderStatus.Pending;
            else if (statusRaw == "ACCEPTED") status = AppPickupOrderStatus.Accepted;
            else if (statusRaw == "PREPARING") status = AppPickupOrderStatus.Preparing;
            else if (statusRaw == "READY") status = AppPickupOrderStatus.Ready;
            else if (statusRaw == "DONE" || statusRaw == "DELIVERED") status = AppPickupOrderStatus.Delivered;
            else status = AppPickupOrderStatus.Pending;

            return new AppPickupOrderRecord
            {
                Id = row.Id,
                CustomerUserId = row.CustomerUserId,
                No = DeriveSupabaseOrderNo(row),
                Name = row.CustomerName ?? string.Empty,
                Phone = row.CustomerPhone ?? string.Empty,
                District = row.District ?? string.Empty,
                AddressText = row.AddressText ?? string.Empty,
                CreatedAt = ParseIsoDateToUnixMs(row.CreatedAt),
                Status = status,
        ...
        private List<AppOrderItemRecord> MapSupabaseItems(List<SupabaseAppOrderItemRow> items, List<SupabaseAppOrderItemRow> metadataItems)
        {
            ...
            if ((items == null || items.Count == 0) && metadataItems != null && metadataItems.Count > 0)
            {
                items = metadataItems;
            }
            ...
            for (int i = 0; i < items.Count; i++)
            {
                var it = items[i];
                if (it == null) continue;
                list.Add(new AppOrderItemRecord
                {
                    Name = it.Name ?? string.Empty,
                    Qty = it.Qty <= 0 ? 1 : it.Qty,
                    Price = (int)Math.Round(it.Price, MidpointRounding.AwayFromZero)
                });
            }
            return list;
        }

        private long ParseIsoDateToUnixMs(string raw)
        {
            if (string.IsNullOrWhiteSpace(raw)) return NowUnixMs();
            DateTimeOffset dto;
        ...
        private int SafeIntFromLong(long value)
        {
