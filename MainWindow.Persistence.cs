...
namespace Fale7_POS
{
    public partial class MainWindow
    {
        ...
        private string TakeawayStateFilePath { get { return Path.Combine(AppDataStateDirPath, "takeaway_state.json"); } }
        private string MenuStateFilePath { get { return Path.Combine(AppDataStateDirPath, "menu_state.json"); } }
        ...
        private void CleanupLegacyLocalFilesOnStartup()
        {
            ...
            TryMigrateLegacyLocalFile("takeaway_state.json", TakeawayStateFilePath);
            TryMigrateLegacyLocalFile("menu_state.json", MenuStateFilePath);
            TryMigrateLegacyLocalFile("cashier_credentials_cache.json", CashierCredentialsCacheFilePath);
            TryMigrateLegacyLocalFile("delivery_state.json", DeliveryStateFilePath);
            TryMigrateLegacyLocalFile("app_products_snapshot.json", AppProductsSnapshotCacheFilePath);
            TryMigrateLegacyLocalFile("pos_offline_orders_queue.json", PosOfflineOrdersQueueFilePath);
            TryMigrateLegacyLocalFile("pos_shift_migration_queue.json", PosShiftMigrationQueueFilePath);
        ...
        {
            _takeawayState = ReadJson<TakeawayStateRecord>(TakeawayStateFilePath) ?? new TakeawayStateRecord();
        }

        private void SaveTakeawayStateToDisk()
        {
            NormalizeTakeawayState();
            WriteJson(TakeawayStateFilePath, _takeawayState);
        }

        private void LoadMenuStateFromDisk()
        {
            if (_menu.Count > 0) return;
            var state = ReadJson<MenuStateRecord>(MenuStateFilePath);
        ...
        private void SaveMenuStateToDisk()
        {
            ...
            WriteJson(MenuStateFilePath, state);
        }
